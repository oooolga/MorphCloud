...
for combo in "${batch_sizes_and_views[@]}"; do
    ...
    python3 \
        ...
        model=morphcloud \
        model/task=non_metric_poses_metric_depth_sparse \
        model.encoder.uses_torch_hub=false \
        model.pretrained='${root_experiments_dir}/morphcloud/training/mapa_curri_24v_13d_36ipg_64g/checkpoint-last.pth' \
        hydra.run.dir='${root_experiments_dir}/morphcloud/benchmarking/dense_'"${num_views}"'_view/mapa_24v_nmp_md_sparse'
