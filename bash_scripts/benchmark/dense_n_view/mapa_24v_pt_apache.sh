...
for combo in "${batch_sizes_and_views[@]}"; do
    ...
    python3 \
        ...
        model=morphcloud \
        model/task=pass_through \
        model.encoder.uses_torch_hub=false \
        model.pretrained='${root_experiments_dir}/morphcloud/training/mapa_curri_24v_6d_36ipg_64g_apache/checkpoint-last.pth' \
        hydra.run.dir='${root_experiments_dir}/morphcloud/benchmarking/dense_'"${num_views}"'_view/mapa_24v_apache_pass_through'
